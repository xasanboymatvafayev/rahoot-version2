...
const inContainerPath = process.env.CONFIG_PATH || "/tmp/rahoot-config"

const getPath = (path: string = "") => resolve(inContainerPath, path)
...
class Config {
  static init() {
    ...
    // Use MANAGER_PASSWORD env var if set, otherwise use default
    ...
    if (!isGameConfigExists) {
      ...
      // Always update password from env var if it's set
      if (process.env.MANAGER_PASSWORD) {
        const existing = JSON.parse(fs.readFileSync(getPath("game.json"), "utf-8"))
        fs.writeFileSync(
          getPath("game.json"),
          JSON.stringify({ ...existing, managerPassword }, null, 2),
        )
      }
    }
